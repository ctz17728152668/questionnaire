...
Questionaire.prototype = {
  init: function () {
    ...
    request.onupgradeneeded = function (event) {
      ...
      objStore.createIndex("count", "count", {//数量
        unique: false
      });
      objStore.createIndex("date", "date", {//日期
        unique: false
      });
      objStore.createIndex("state", "state", {//状态
        unique: false
      });
      objStore.createIndex("allData", "allData", {//截至日期
        unique: false
      });
          ...
          if (cursor) {
            switch (cursor.value.state) { //判断状态
              ...
              case "0":
                stateText = "发布中";
                stateClass = "pubing";
                inputStyle = "<a href='write.html?subject=" + encodeURI(cursor.value.subject) + "'><input type='button' value='填写问卷'></a><a href='detail.html?subject=" + encodeURI(cursor.value.subject) + "'><input type='button' value='查看数据'></a>";
                break;
              case "1":
                stateText = "已结束";
                stateClass = "pubed";
            ...
            //渲染HTML内容
            htmlCont += "<tr><td><input type='checkbox' name='myQN' onchange='myQuestionaire.index.toggleCheck(event)' ></td><td>" + cursor.value.subject + "</td><td>" + cursor.value.date + "</td><td class='" + stateClass + 
			"'>" + stateText + "</td><td id=" + cursor.value.subject + ">" + inputStyle + "</td></tr>";
  ...
  edit: {
    ...
    OBJtoHTML: function (obj) { //接收IDB读取出来的OBJ
      ...
      for (var i = 0; i < obj.allData.length; i++) {
        ...
        for (var j = 0; j < obj.allData[i].data.length; j++) {
          switch (obj.allData[i].type) {
            case "radio":
              html += "<div class='radios'><input type='radio' name='" + Math.random() + "'  disabled='disabled'><input type='text' placeholder='单选内容' value='" + obj.allData[i].data[j].content + "'><span onclick='myQuestionaire.edit.deleSelf(event)' class='dele-self'>删除</span></div>";
              break;
            case "checkbox":
              html += "<div class='checkboxs'><input type='checkbox'  disabled='disabled'><input type='text' placeholder='多选内容' value='" + obj.allData[i].data[j].content + "'><span onclick='myQuestionaire.edit.deleSelf(event)' class='dele-self'>删除</span></div>";
              break;
            case "textarea":
              if (obj.allData[i].data[j].status == "true") { //checked属性，想把勾选状态改成不勾选状态，貌似只有删除checked属性。把checked设置成什么东西，都会勾上。只有再加一层if了
    ...
    topPart: function (type) {
      ...
      switch (type) {
        case "radio":
          div.className = "radio";
          html = "<span></span><span>单选题</span><input type='text' class='subject' placeholder='单选题'>";
          div.innerHTML = html;
          break;
        case "checkbox":
          div.className = "checkbox";
          html = "<span></span><span>多选题</span><input type='text' class='subject' placeholder='多选题'>";
          div.innerHTML = html;
          break;
        case "textarea":
          div.className = "textarea";
          html = "<span></span><span>文本题</span><input type='text' class='subject' placeholder='文本题'>";
          div.innerHTML = html;
          break;
        default:
          break;
    ...
    middlePart: function (type, randomNumber) {
      ...
      switch (type) {
        case "radio":
          html = "<div class='radios'><input type='radio' name='radio" + randomNumber + "' disabled><input type='text' placeholder='单选内容' ></div><br><div class='radios'><input type='radio' name='radio" + randomNumber + "' disabled><input type='text' placeholder='单选内容' ></div><br><div class='radios'><input type='radio' name='radio" + randomNumber + "' disabled><input type='text' placeholder='单选内容' ></div>";
          divParent.innerHTML = html;
          return divParent;

        case "checkbox":
          html = "<div class='checkboxs'><input type='checkbox' disabled' /><input type='text' placeholder='多选内容'></div><br><div class='checkboxs'><input type='checkbox' disabled' /><input type='text' placeholder='多选内容'></div><br><div class='checkboxs'><input type='checkbox' disabled' /><input type='text' placeholder='多选内容'></div><br><br><div class='checkboxs'><input type='checkbox' disabled' /><input type='text' placeholder='多选内容'></div><br>";
          divParent.innerHTML = html;
          return divParent;
